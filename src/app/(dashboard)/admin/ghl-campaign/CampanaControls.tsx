...
export function CampanaControls({ activa, pendientes }: Props) {
  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      {/* R7: utilidades separadas del toggle principal */}
      <div className="flex items-center gap-2">
        {/* Auto-actualizar */}
        <button
          onClick={toggleAutoRefresh}
          title={autoRefresh ? "Desactivar auto-actualización" : "Activar auto-actualización cada 15 s"}
          className={`inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border px-3 text-xs font-medium transition-all cursor-pointer
            ${autoRefresh
        ...
        {/* R6: "Cobertura IA" en lugar de "Auditar cobertura" */}
        <button
          ...
          className={`inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border px-3 text-xs font-medium text-muted-foreground hover:bg-muted/60 transition-all
            ${pendingAudit ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
        >
          {pendingAudit ? "Analizando…" : "Cobertura IA"}
        </button>

        {/* R3: Reinicio con color de peligro */}
        <button
          ...
          className={`inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border px-3 text-xs font-medium transition-all
            border-red-400/40 text-red-600 dark:text-red-400 hover:bg-red-500/10
            ${pendingReset ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
        >
          {pendingReset ? "Reiniciando…" : "↺ Reinicio"}
        </button>
      </div>

      {/* R7: Toggle campaña separado — es la acción principal */}
      <button
