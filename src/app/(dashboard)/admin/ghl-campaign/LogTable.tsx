...
type Filtro = "todos" | "sin_respuesta" | "interesado" | "negativo" | "convertido";
...
function aplicarFiltro(rows: LogRow[], filtro: Filtro): LogRow[] {
  switch (filtro) {
    case "sin_respuesta": return rows.filter((r) => !r.respuesta_tipo && r.enviado);
    case "interesado":    return rows.filter((r) => r.respuesta_tipo && !r.convirtio && r.respuesta_tipo !== "negativo");
    case "negativo":      return rows.filter((r) => r.respuesta_tipo === "negativo");
    case "convertido":    return rows.filter((r) => r.convirtio === true);
    default:              return rows;
  }
}
...
export function LogTable({ logs: inicial }: { logs: LogRow[] }) {
  ...
  const visibles = aplicarFiltro(rows, filtro);
        ...
        <div className="border-t px-4 pb-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5 pt-3">
            {FILTROS.map(({ key, label }) => {
              const count = key === "todos" ? rows.length : aplicarFiltro(rows, key).length;
              return (
                <button
                  key={key}
                ...
                >
                  {label}
                  <span className={`tabular-nums ${filtro === key ? "opacity-80" : "opacity-60"}`}>
                    {count}
                  </span>
                </button>
              );
          ...
          {visibles.length === 0 ? (
            <p className="text-sm text-muted-foreground py-2">Sin registros para este filtro.</p>
          ) : (
